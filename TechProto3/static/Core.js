...
var display_video = function(num)
{
    ...
    $.each(Core, function(i, value){
        ...
        //var row = $("<div class = 'row bottom_row_padding'>")
                ...
                //$(col_entry).append("<div id='name'> "+Core[i]['name']+"</div>");
                //$(row).append(col_entry)
                console.log(Core[i])
                console.log(Core[i]['benefits'])
                console.log(Core[i]['name'])
                //var col_entry = $("<div class = 'col-md-3'>")
                
                 $(col_entry).append("<div class = 'vid-container' <span id='"+Core[i]["video_link"]+"'> <span id ='name'>"+Core[i]['name']+"</span></span> <iframe class='video' id = '"+Core[i]["benefits"]+"' width='150' height='100' src='https://www.youtube.com/embed/?playlist="+Core[i]["video_link"]+"'></iframe> <button class ='btn btn-outline-secondary select_button " + item + "'>select</button></div>")
                 //$(row).append(col_entry)
