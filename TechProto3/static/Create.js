...
var display_video = function(num)
{
    ...
    $.each(warmupposes, function(i, value){
        ...
        //var row = $("<div class = 'row bottom_row_padding'>")
                ...
                //$(col_entry).append("<div id='name'> "+warmupposes[i]['name']+"</div>");
                //$(row).append(col_entry)
                console.log(warmupposes[i])
                console.log(warmupposes[i]['benefits'])
                console.log(warmupposes[i]['name'])
                //var col_entry = $("<div class = 'col-md-3'>")
                
                 $(col_entry).append("<div class = 'vid-container' <span id='"+warmupposes[i]["video_link"]+"'> <span id ='name'>"+warmupposes[i]['name']+"</span></span> <iframe class='video' id = '"+warmupposes[i]["benefits"]+"' width='150' height='100' src='https://www.youtube.com/embed/?playlist="+warmupposes[i]["video_link"]+"'></iframe> <button class ='btn btn-outline-secondary select_button " + item + "'>select</button></div>")                 //$(row).append(col_entry)
   ...
   $(".select_button").click(function()
    ...
    $(".Next").click(function(){  
        if (integer1 > 0)
        {
            ...
            //SEND selected_pose
             localStorage.setItem('warmuppose',selected_pose);
            //surrenderpose=selected_pose;
            console.log("warmuppose POSE")
            //console.log(surrenderpose)
            window.location.assign("/Integration")
        }else
        {
