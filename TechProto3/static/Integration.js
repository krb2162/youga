...
var display_video = function(num)
{
    ...
    console.log(num)
    console.log(Stretch0[1]["name"])
    var poses = ["IUqcLGOEzPw","hmZxKCXS0tY","nZC4qflcsgk","yqgsm8wzcJw","-r15RxP8ZzE","5vIJX7SIiJo"]
    $.each(Stretch0, function(i, value){
        //console.log(i)
        //console.log(poses[i])
        // console.log(poses[1])
        // console.log(poses[2])
        ...
        //var row = $("<div class = 'row bottom_row_padding'>")
                ...
                //$(col_entry).append("<div id='name'> "+Stretch0[i]['name']+"</div>");
                //$(row).append(col_entry)
                console.log(Stretch0[i])
                console.log(Stretch0[i]['benefits'])
                console.log(Stretch0[i]['name'])
                //var col_entry = $("<div class = 'col-md-3'>")
                
                 $(col_entry).append("<div class = 'vid-container' <span id='"+Stretch0[i]["video_link"]+"'> <span id ='name'>"+Stretch0[i]['name']+"</span></span> <iframe class='video' id = '"+Stretch0[i]["benefits"]+"' width='150' height='100' src='https://www.youtube.com/embed/?playlist="+Stretch0[i]["video_link"]+"'></iframe> <button class ='btn btn-outline-secondary select_button " + item + "'>select</button></div>")
                 //$(row).append(col_entry)
   ...
   $(".select_button").click(function()
    ...
    $(".Next").click(function(){  
        if (integer1 > 0)
        {
            ...
            //SEND selected_pose
             localStorage.setItem('integrationpose',selected_pose);
            //surrenderpose=selected_pose;
            console.log("integrationpose POSE")
            //console.log(surrenderpose)
            window.location.assign("/SunA")
        }else
        {
