...
var display_video = function(num)
{
    ...
    $.each(Arm, function(i, value){
        ...
        //var row = $("<div class = 'row bottom_row_padding'>")
                ...
                //$(col_entry).append("<div id='name'> "+Arm[i]['name']+"</div>");
                //$(row).append(col_entry)
                console.log(Arm[i])
                console.log(Arm[i]['benefits'])
                console.log(Arm[i]['name'])
                //var col_entry = $("<div class = 'col-md-3'>")
                
                 $(col_entry).append("<div class = 'vid-container' <span id='"+Arm[i]["video_link"]+"'> <span id ='name'>"+Arm[i]['name']+"</span></span> <iframe class='video' id = '"+Arm[i]["benefits"]+"' width='150' height='100' src='https://www.youtube.com/embed/?playlist="+Arm[i]["video_link"]+"'></iframe> <button class ='btn btn-outline-secondary select_button " + item + "'>select</button></div>")
                 //$(row).append(col_entry)
