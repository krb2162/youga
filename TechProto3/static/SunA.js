...
var display_video = function(num)
{
    ...
    $.each(SunAflow, function(i, value){
        ...
        //var row = $("<div class = 'row bottom_row_padding'>")
                ...
                //$(col_entry).append("<div id='name'> "+SunAflow[i]['name']+"</div>");
                //$(row).append(col_entry)
                console.log(SunAflow[i])
                console.log(SunAflow[i]['benefits'])
                console.log(SunAflow[i]['name'])
                //var col_entry = $("<div class = 'col-md-3'>")
                
                 $(col_entry).append("<div class = 'vid-container' <span id='"+SunAflow[i]["video_link"]+"'> <span id ='name'>"+SunAflow[i]['name']+"</span></span> <iframe class='video' id = '"+SunAflow[i]["benefits"]+"' width='150' height='100' src='https://www.youtube.com/embed/?playlist="+SunAflow[i]["video_link"]+"'></iframe> <button class ='btn btn-outline-secondary select_button " + item + "'>select</button></div>")
                 //$(row).append(col_entry)
   ...
   $(".select_button").click(function()
    ...
    $(".Next").click(function(){  
        if (integer1 > 0)
        {
            ...
            //SEND selected_pose
             localStorage.setItem('sunA',selected_pose);
